...
const defaultConfig = <F extends Filter>(filter: F): FilterConfigMap[F] => {
  switch (filter) {
    ...
    case Filter.AsSound:
      return { blend: 0.5 } as FilterConfigMap[F]
    case Filter.FractalPixelSort:
      return {
        intensity: 6.0,
        distortedData: new Uint8ClampedArray()
      ...
      const updatedLayers = state.layers.map((layer) => {
        ...
        switch (filter) {
          case Filter.AsSound: {
            ...
            const wavBytes = audioSamplesToWAV(distortedSamples, DEFAULT_SAMPLE_RATE)
            ...
            for (const { x, y } of updatedSelection.area) {
              ...
              data[index] = data[index] * (1 - bitRateBlend) + glitchedValue * bitRateBlend // R
              data[index + 1] = data[index + 1] * (1 - bitRateBlend) + glitchedValue * bitRateBlend // G
              data[index + 2] = data[index + 2] * (1 - bitRateBlend) + glitchedValue * bitRateBlend // B
            }
